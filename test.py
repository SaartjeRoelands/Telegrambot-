...
SEND = 'sendMessage'
GETUPDATE = 'getUpdate'
#PARAMS = {'chat_id':1261249355, 'text': 'test + test'} 
ID_SAARTJE = 1261249355
ID_JEROEN = 878864710 
OFFSET = 0
    ...
    if (data["ok"]):
        for i in range(len(data["result"])):
            if((data["result"][i]["update_id"]) >= OFFSET):
                ...
                #print(OFFSET)
            if(data["result"][i]["message"]["chat"]["id"] == ID_SAARTJE):
                text = data["result"][i]["message"]["text"]
                #print(text)
                if ("/getTemperature" in text):
                    await replyTemperature(ID_SAARTJE)
            if(data["result"][i]["message"]["chat"]["id"] == ID_SAARTJE):
                #print(text)
                if ("/getTemperature" in text):
                    await replyTemperature(ID_SAARTJE)
...
def send_chat(CONTENT):
    PARAMS = {'chat_id':1261249355 , 'text': CONTENT}
    r = requests.get(url = URL + SEND, params = PARAMS) 
